...
\begin{abstract}
\noindent
Connectome-constrained network models of the \textit{Drosophila} ventral nerve cord (VNC) now reproduce rhythmic premotor activity in response to descending drive, inviting the next step of coupling them to a physical body. We asked whether the published rate model of the full male adult nerve cord (MANC; 23,532 neurons, 1,372,404 signed connections) can serve as such a controller, and organised the attempt as a sequence of falsification tests. Across all 128 author-saved parameter sets driven by bilateral DNg100 input, 51 produced sustained ${\sim}11$\,Hz rhythm in all six E1 premotor neurons, yet none of these showed co-modulated tibia flexor and extensor output in any leg: all 17 left-front tibia motor neurons were silent in every rhythmic set, and in the five sets analysed in detail they remained 4.4--5.0 model units below threshold although their incoming weights were identical to those of a recruiting set. The ensemble split into two regimes separated by a gap in motor-neuron activity ($\leq$38 versus $\geq$289 of 732 motor neurons active); antagonistic tibia output occurred only in the hyperactive regime (9 of 10 sets versus 0 of 118; Fisher's exact $p=5\times10^{-13}$), in which E1 rhythm was absent or confined to at most two hemisegments. Tonic current into a flexor premotor neuron recruited a few flexor motor neurons without abolishing the rhythm, but their output was unmodulated and the extensors stayed silent; closing a sensory loop with a physical body left all front- and middle-leg tibia pools subthreshold; and replaying the saved motor output into two six-legged bodies produced no walking (0/256 runs). We also report four pitfalls, each of which produced a false positive or a false negative during the project: (i) a hybrid walker that passed a locomotion gate was bit-identical when the online network was replaced by a replayed phase signal; (ii) fixed-step Euler integration created a spurious candidate in a sub-network and changed rhythm classification in 6 of 32 full-network sets (one still misclassified at 0.1\,ms steps), whereas adaptive Dormand--Prince integration reproduced the archived solutions; (iii) a displacement-plus-toe-height gait gate accepted a controller that was dragging its feet; and (iv) a peak-based rhythm detector with a 100\,ms minimum peak spacing undercounted ${\sim}11$\,Hz oscillations tenfold (5 versus 51 sets). We propose these controls as minimal acceptance tests for connectome-to-body claims and identify motor-neuron recruitment, not rhythm generation, as the current bottleneck in this model class.
\end{abstract}
